...
class CreditCardController extends Controller
{
 ...
 {
  $model = $this->creditCardService->find($id);
  ...
  if ($request->isMethod('post')) {
   ...
   if (method_exists($model, 'getErrors') && $model->getErrors()) {
    ...
    $request->session()->flash('success', ['message' => 'Cartão de crédito alterado com sucesso.']);
   }
   return redirect("credit-card/");

  }
  $title  = 'Cartão de crédito';
  $action = "/credit-card/update/{$id}";

  $bankAccounts     = BankAccount::all();
  $creditCardBrands = CreditCardBrand::all();
  $model            = (object) fractal($model, new CreditCardTransformer)->toArray()['data'];
